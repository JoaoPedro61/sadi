...
async fn main() -> Result<(), String> {
    ...
    let user2 = create_user
        .execute("Bob".to_string(), "bob@example.com".to_string())
        .await?;
    println!("✓ Created user: {:?}", user2);

    let user3 = create_user
        .execute("Charlie".to_string(), "charlie@example.com".to_string())
        .await?;
    println!("✓ Created user: {:?}\n", user3);

    // Get all users
    let get_all_users = app
        .injector()
